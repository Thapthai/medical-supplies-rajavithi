...
/** ยี่ห้อจาก ITEM_BRANDS ใน .env (คั่นด้วย comma) */
export function getKnownItemBrands(): string[] {
  return parseItemBrandsEnv(process.env.ITEM_BRANDS);
}

/** @deprecated ใช้ getKnownItemBrands() — คงไว้เพื่อ import เดิม */
...
function matchKnownBrand(name: string, knownBrands: string[]): string | null {
  ...
  for (const brand of ordered) {
    ...
    if (re.test(name)) {
      // คืนค่าตามที่ตั้งใน env (เคสเดิม)
      return brand;
    }
  }
  return null;
...
/**
 * ดึงยี่ห้อจากชื่ออุปกรณ์ ตาม ITEM_BRANDS ใน .env
...
/** ส่วนหลัง " + " (เช่น "14.5 D") — ไม่มี separator คืน null */
