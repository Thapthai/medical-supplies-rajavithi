...
export default function PrePrintOrderCard({
  preparedLines,
  emptyHint = 'กดปุ่มเตรียมพิมจากรายการอุปกรณ์เพื่อแสดงที่นี่',
  onSetCopies,
  onExpireDateChange,
  onRemoveLine,
  onClearAll,
  ...
  const totalSheets = preparedLines.reduce(
    (s, l) => s + resolveCopies(l.copies, l.refillCap),
    0,
  );
          ...
          <>
            ...
            <div className="max-h-[min(58vh,calc(100dvh-16rem))] space-y-2 overflow-y-auto overscroll-contain md:hidden">
              {groupedLines.map(([itemcode, lines]) => {
                const head = lines[0];
                const groupSheets = lines.reduce(
                  (s, l) => s + resolveCopies(l.copies, l.refillCap),
                  0,
                );
                return (
                          ...
                          item={
                            {
                              ...
                              SubUnitQty: head.SubUnitQty,
                              unit: head.unit,
                              subUnit: head.subUnit,
                            } as Item
                          }
                          qtyMain={groupSheets}
                        />
            ...
            {/* Desktop: group by item — name once, lots below */}
