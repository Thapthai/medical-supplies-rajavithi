...
export default function PrePrintHistoryTab({ refreshKey = 0 }: PrePrintHistoryTabProps) {
  ...
  const handleExportReport = async (format: 'excel' | 'pdf') => {
    ...
    try {
      ...
      if (format === 'excel') {
        await stickerPrintApi.downloadPrePrintStickersReportExcel(params);
      } else {
        await stickerPrintApi.downloadPrePrintStickersReportPdf(params);
      }
      toast.success(`ดาวน์โหลด ${format.toUpperCase()} สำเร็จ`);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'ส่งออกไม่สำเร็จ';
      toast.error(msg);
    } finally {
